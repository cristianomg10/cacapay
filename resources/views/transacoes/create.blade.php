...
@section('header')
    <h2 class="h4 fw-bold mb-0">{{ __('Nova Transação') }}</h2>
@endsection

@section('content')
    <div class="card shadow-sm">
        <div class="card-header bg-white">
            <h5 class="card-title mb-0 fw-semibold">Nova Transação</h5>
        </div>
        <div class="card-body">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('empresa_parceira_id') is-invalid @enderror" id="empresa_parceira_id" name="empresa_parceira_id" required>
                        ...
                        @foreach ($empresasParceiras as $empresa)
                            <option value="{{ $empresa->id }}" {{ old('empresa_parceira_id') == $empresa->id ? 'selected' : '' }}>{{ $empresa->razao_social }}</option>
                        @endforeach
                    </select>
